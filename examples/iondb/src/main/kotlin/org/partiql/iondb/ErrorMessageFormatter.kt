...
object ErrorMessageFormatter {
    fun message(error: PError): String {
        ...
        val content = when (errorCode) {
            PError.UNEXPECTED_TOKEN -> unexpectedToken(error)
            PError.UNRECOGNIZED_TOKEN -> unrecognizedToken(error)
            PError.FUNCTION_TYPE_MISMATCH -> fnTypeMismatch(error)
            PError.FUNCTION_NOT_FOUND -> fnTypeMismatch(error) // TODO: Add dedicated message for this.
            PError.UNDEFINED_CAST -> undefinedCast(error)
            PError.INTERNAL_ERROR -> internalError(error) // TODO: Make the verbosity a variable
            PError.PATH_KEY_NEVER_SUCCEEDS -> pathNeverSucceeds("key")
            PError.PATH_SYMBOL_NEVER_SUCCEEDS -> pathNeverSucceeds("symbol")
            PError.PATH_INDEX_NEVER_SUCCEEDS -> pathNeverSucceeds("index")
            PError.ALWAYS_MISSING -> alwaysMissing()
            PError.FEATURE_NOT_SUPPORTED -> featureNotSupported(error)
            PError.VAR_REF_AMBIGUOUS -> varRefAmbiguous(error)
            PError.VAR_REF_NOT_FOUND -> varRefNotFound(error)
            PError.UNKNOWN -> "Unrecognized error code received: ${error.code()}"
            else -> "Unrecognized error code received: ${error.code()}"
        }
        return buildString {
