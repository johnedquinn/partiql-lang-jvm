...
object ServiceLoaderUtil {
    ...
    @OptIn(PartiQLFunctionExperimental::class)
    @JvmStatic
    fun loadPlugins(): List<Plugin> = lock.withLock {
        val plugins = ServiceLoader.load(Plugin::class.java)
    ...
    //        @JvmStatic
    //        fun loadBindings(session: EvaluationSession): Bindings<ExprValue> {
    //            val metadata = lock.withLock {
    //                val plugins = ServiceLoader.load(Plugin::class.java)
    //                plugins
    //                    .flatMap { plugin -> plugin.getConnectorFactories() }
    //                    .map { factory -> factory.create("aws", emptyIonStruct()) }
    //                    .map { connector -> connector.getMetadata(session.toConnectorSession()) }.first()
    //            }
    //            return object : Bindings<ExprValue> {
    //                override fun get(bindingName: BindingName): ExprValue {
    //                    val path = bindingName.name.let { BindingPath(listOf(org.partiql.spi.BindingName(it, BindingCase.INSENSITIVE))) }
    //                    val value = metadata.getValue(session.toConnectorSession(), path)
    //                    return PartiQLtoExprValue(value)
    //                }
    //            }
    //        }

    @OptIn(PartiQLValueExperimental::class, PartiQLFunctionExperimental::class)
    internal fun PartiQLtoExprFunction(customFunction: PartiQLFunction): List<ExprFunction> {
        val names = customFunction.signature.names
        val parameters =
            customFunction.signature.parameters.filterIsInstance<PartiQLFunction.Parameter.ValueParameter>()
                .map { it.type }
                ...
                object : ExprFunction {
                    ...
                    override fun callWithRequired(session: EvaluationSession, required: List<ExprValue>): ExprValue {
                        val partiQLArguments =
                            required.mapIndexed { i, expr -> ExprToPartiQLValue(expr, parameters[i]) }
    ...
    private fun PartiQLtoExprValue(partiqlValue: PartiQLValue): ExprValue {
        return when (partiqlValue.type) {
            ...
            PartiQLValueType.STRUCT -> {
                (partiqlValue as? StructValue<*>)?.fields
                    ?.map { PartiQLtoExprValue(it.second).namedValue(newString(it.first)) }
